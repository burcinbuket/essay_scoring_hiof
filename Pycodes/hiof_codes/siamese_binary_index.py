...
output_dir = output_dir + f"/llama_embeddings_binary/val__{timestamp}"
...
# === Load CSV ===
# dset is located under these paths
...
# === Filter out class 0 (equal quality essays) ===
embed_df = embed_df[embed_df["true_label"] != 0].reset_index(drop=True)

# === Extract features and labels ===
X1 = embed_df.iloc[:, 2:2050].astype(np.float32).values
X2 = embed_df.iloc[:, 2050:4098].astype(np.float32).values
y_raw = embed_df["true_label"].values  # only 1 or 2 now

# Adjust labels: 1 -> 0 (essay1 better), 2 -> 1 (essay2 better)
y_binary = np.array([0 if y == 1 else 1 for y in y_raw])
...
output = Dense(2, activation="softmax", name="classification")(diff)
...
results_df = pd.DataFrame({
    ...
    "prob_essay2_better": predictions[:, 1],
})
...
pd.DataFrame(conf_matrix,
             index=["True_essay1_better", "True_essay2_better"],
             columns=["Pred_essay1_better", "Pred_essay2_better"]).to_csv(output_dir + "/csvs/confusion_matrix.csv")



# Use only class 1's probability (essay 2 better)
...
plt.title('ROC Curve - Essay2 Better as Positive Class')
...
K.clear_session()
print("✅ Binary classification complete. Results saved to:", output_dir)
